 //控制层 ，引入brandService，调用其中查询品牌下拉列表的方法selectOptionList()
app.controller('typeTemplateController' ,function($scope,$controller,brandService,specificationService,typeTemplateService){
	...
	$scope.findOne=function(id){				
		typeTemplateService.findOne(id).success(
			function(response){
				...
				//将品牌列表的JSON字符串转换成对象，必须要转换成对象才能显示到页面当中
	...
	//创建一个变量，用于接收后端传来的品牌下拉数据
	$scope.brandList={data:[]};
	//读取品牌列表
	$scope.findBrandList=function(){
		brandService.selectOptionList().success(
			function(response){
				//后端查到的数据是：[{"id":1,"text":"联想"},{"id":3,"text":"三星"},{"id":2,"text":"华为"}]
