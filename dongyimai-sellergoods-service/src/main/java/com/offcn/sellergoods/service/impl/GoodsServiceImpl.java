...
public class GoodsServiceImpl implements GoodsService {
	...
	private void saveItemList(Goods goods){
		if("1".equals(goods.getGoods().getIsEnableSpec())){//如果启用规格
			...
			//由于不启用规格，所以就没有价格、状态、库存、是否默认、规格选项等字段，所以给它定义一个默认的值
	...
	public void delete(Long[] ids) {
		...
		//修改商品sku状态为禁用
		List<TbItem> listitem = findItemListByGoodsIdandStatus(ids,"1");//查找出所有审核通过的商品
...
//				criteria.andIsDeleteLike("%"+goods.getIsDelete()+"%");
							criteria.andIsDeleteIsNull();//非删除状态 null是没有被删除，"1"是被删除
//			}
